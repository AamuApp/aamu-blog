...
modified: "2025-05-19T07:49:22.645Z"
description: "How to use the database from a distance"
cover:
  image: 5473837549387959_GraphQL Logo + Wordmark (Rhodamine).png
...
}</code></pre><p>When you do the query, you will get the data back (if it exists) as json:</p><pre><code>{
  "data": {
    "BlogPost": {
...
}</code></pre><p>This will give the following results:</p><pre><code>{
  "data": {
    "BlogPostCollection": [
      {
        "title": "Introduction to Aamu.app GraphQL",
...
}</code></pre><p>You can use other filtering methods, for example <strong>GT</strong> (Greater Than):</p><pre><code>query {
    BlogPostCollection(
        filter: {
            status: { EQ: "published" }
