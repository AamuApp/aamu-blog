...
<h2>Tasks as an API-backed reference app</h2>
<p><a href="https://github.com/AamuApp/my-app-tasks">AamuApp/my-app-tasks</a> is a second open-source reference application. It shows how a My Apps application can combine the launch context with a separately granted Tasks API key to list, create, and edit Aamu tasks in its own focused interface.</p>
<p>The app supports task status, assignees, start and due dates, repetition, reminders, Markdown bodies, comments, and real Aamu user mentions. It uses the My Apps team context to resolve users while task data remains in Aamu through the Tasks API.</p>
<p>The implementation uses SvelteKit, TypeScript, and a small SQLite session store. Together with Kudos, it demonstrates both sides of the extension model: an app can own a separate workflow and send events back to Aamu, or it can provide a purpose-built interface over data owned by an Aamu API.</p>
<p>Reference implementation: <a href="https://github.com/AamuApp/my-app-tasks">AamuApp/my-app-tasks on GitHub</a>.</p>
